...
class Parser:
    def __init__(self, version: str):
        self.mappings = {
            ...
            "gsm": ["exact_match_indicator", "inference_runtime"],
            "truthful_qa": ["exact_match", "exact_match+fairness", "exact_match+robustness", "inference_runtime"],
            "big_bench": ["exact_match", "exact_match+fairness", "exact_match+robustness", "inference_runtime"],
            "summarization_cnndm": [
                "rouge_2",
    ...
    def show_leaderboard(self):
        ...
        template = f"""
        BigBench - EM                              {parsed_data['big_bench']['exact_match']}
        ...
        GSM8K - EM                                 {parsed_data['gsm']['exact_match_indicator']}
        BigBench - EM (Robustness)                 {parsed_data['big_bench']['exact_match+robustness']}
        ...
        BigBench - EM (Fairness)                   {parsed_data['big_bench']['exact_match+fairness']}
        ...
        BigBench - Idealized inference time (s)    {parsed_data['big_bench']["inference_runtime"]}
        MMLU - Idealized inference time (s)        {parsed_data['mmlu']["inference_runtime"]}
        TruthfulQA - Idealized inference time (s)  {parsed_data['truthful_qa']["inference_runtime"]}
        BBQ - Idealized inference time (s)         {parsed_data['bbq']["inference_runtime"]}
        GSM8K - Idealized inference time (s)       {parsed_data['gsm']["inference_runtime"]}
        """
        print(template)
    ...
    def parse_raw_data(self):
        ...
        for metric in self.mappings['mmlu']:
            mmlu_data[metric] = []
        for metric in self.mappings['big_bench']:
            big_bench_data[metric] = []

        ptr_maps = {'mmlu': mmlu_data, 'big_bench': big_bench_data}
        keyword_split = {'mmlu': 'subject', 'big_bench': 'task'}
    ...
    def extract_data(self, name: str, path: str, metrics: List[str]):
        if name in ['mmlu', 'bbq', 'gsm', 'summarization_cnndm', 'big_bench']:
